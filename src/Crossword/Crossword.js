...
function createGridData(data) {
  ...
  const size = Math.max(rowMax, colMax) + 1;
  ...
  // TODO: These two loops are *soooo* similar... any way to refactor?
  Object.entries(data.across).forEach(([number, info]) => {
    const { row, col, clue, answer } = info;
    for (let i = 0; i < answer.length; i++) {
      const cellData = gridData[row][col + i];

      // TODO?: check to ensure the answer is the same if it's already set?
      cellData.used = true;
      cellData.answer = answer[i];
      cellData.across = number;
    ...
    clues.across.push({ number, clue });
  });

  clues.across.sort(byNumber);

  Object.entries(data.down).forEach(([number, info]) => {
    const { row, col, clue, answer } = info;

    for (let i = 0; i < answer.length; i++) {
      const cellData = gridData[row + i][col];

      // TODO?: check to ensure the answer is the same if it's already set?
      cellData.used = true;
      cellData.answer = answer[i];
      cellData.down = number;
    ...
    clues.down.push({ number, clue });
  });

  clues.down.sort(byNumber);
